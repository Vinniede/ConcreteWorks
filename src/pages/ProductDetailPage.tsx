...
export const ProductDetailPage: React.FC = () => {
  ...
  return (
    <div className="product-detail-container">
      <Link to="/products" style={{ color: 'var(--accent-orange)', marginBottom: '2rem', display: 'inline-block' }}>
        ← Back to Products
      </Link>
              ...
              <div className="spec-value">
                <ul className="features-list" style={{ marginBottom: 0 }}>
                  {product.useCases.map((useCase, idx) => (
                    <li key={idx} style={{ borderBottom: idx === product.useCases.length - 1 ? 'none' : '1px solid var(--gray-border)' }}>
                      {useCase}
                    </li>
                  ))}
                </ul>
